...
@Tag(name = "TopUp controller", description = "Операции с пополнениями баланса")
...
public class TopUpRestControllerV1 {
    ...
    public Mono<?> getTopUpTransactionById(ServerWebExchange webExchange, @PathVariable @Parameter(description = "Идентификатор транзакции",
            in = ParameterIn.PATH, name = "id", required = true, schema = @Schema(
            defaultValue = "1",
            minimum = "1",
            allOf = {String.class}),
            style = ParameterStyle.SIMPLE) String id) {
        return transactionService.findTransactionByIdAndMerchantId(webExchange.getAttribute(ApplicationConstants.MERCHANT_ID), UUID.fromString(id), TransactionType.TOP_UP)
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Transaction topUp created",
                    content = {@Content(mediaType = "application/json",
            ...
            @ApiResponse(responseCode = "400", description = "Transaction failed",
                    content = @Content)})
    @PostMapping("/")
    public Mono<?> createTransaction( ServerWebExchange webExchange, @RequestBody
                                     @Parameter(description = "Transaction for top up balance",
                                             schema = @Schema(additionalPropertiesSchema = TransactionDto.class)) TransactionDto transactionDto) {
        return transactionService.topUp(transactionMapper.map(transactionDto), webExchange.getAttribute(ApplicationConstants.MERCHANT_ENTITY));
    }
    ...
    @GetMapping("/lists")
    public Flux<?> getAllTopUpTransactions(ServerWebExchange webExchange,
        ...
        return transactionService.findAllByDateAndAccountId(webExchange.getAttribute(ApplicationConstants.MERCHANT_ENTITY), dateStart, exitDate, TransactionType.TOP_UP).map(transactionMapper::map);
    }
